...
class BonesDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if img_name == 'new_data/BONEAGE_P2_08930.png' :
        #     print(img_name)
        #     merge.show()
        #     exit()
        ...
        with torch.no_grad():
            for batch in tqdm(data_loader):
                ...
                for i in modeldir:
                    ...
                    preds = torch.where(outputs >= 0.4, torch.tensor(1.0, device=device), torch.tensor(0.0, device=device))  #hard
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader):
            ...
            for i in modeldir:
                ...
                smallmodel = torch.load(i)
                smallmodel.eval()
                outputs = smallmodel(images, gender)
                outputs = torch.sigmoid(outputs)
                batch_probs.append(outputs)
                # print(preds.squeeze())
                # print(gt.squeeze())
            avg_prob = torch.mean(torch.stack(batch_probs), dim=0)                               #soft
            # print(batch_predictions[1])
            # exit()
            final_batch_preds = torch.where(avg_prob >= 0.4, torch.tensor(1.0, device=device), torch.tensor(0.0, device=device))        #soft 
            ...
            total += gt.size(0)
            running_acc += (final_batch_preds == gt).sum().item()                                                                 #soft
            # print(final_batch_preds,gt.squeeze())
            # print(running_acc)
            # exit()
            # running_acc += (preds == gt).sum().item()
...
modelpath = './ablation/tall/xray+gender/'
pattern = modelpath + 'model_ablation_xray_gender_subdataset*.pth'
...
# 繪製10個基礎模型的ROC
for i in range(1, 11):
   metrics = np.load(f'./ablation/tall/xray+gender/tall_model_{i}_val_metrics_ablation_xray_gender.npy', allow_pickle=True)
...
# print(result_df)
# result_df.to_csv("prediction_tall.csv")
...
plt.grid(True)
plt.savefig("ROC_tall_softvoting_ablation_xray_gender.png")
plt.show()
...
plt.savefig("voting_cm_tall_ablation_xray_gender.png")
plt.show()