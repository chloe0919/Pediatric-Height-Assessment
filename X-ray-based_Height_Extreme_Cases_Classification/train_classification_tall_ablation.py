...
#         z = self.cat_fc(z)
#         z = self.cat_relu(z)
#         # age_gender = torch.cat((y, a), dim=1)
#         # age_gender_features = self.dnn_age_gender(age_gender)
#         # print(out.shape)
#         # exit()
#         # z = torch.cat((out, age_gender_features), dim=1)
...
#         return z 
# bone_age_model = torch.load('ncku_baprediction_xrayonly_epoch300_mae6.2.pth')
# bone_age_model.eval()
# bone_age_model = torch.nn.Sequential(*list(bone_age_model.children())[:-4])
class CombinedModel(nn.Module):
    ...
    def forward(self, xray_image, y):
        ...
        #--------------------------------------------------------------
        gender_one_hot = F.one_hot(y, num_classes=2).float()
        # # age_one_hot = F.one_hot(a, num_classes=18).float()
        ...
        return z 
class CombinedModel_gender(nn.Module):
    def __init__(self, bone_age_model_path, num_classes):
        super(CombinedModel_gender, self).__init__()
    ...
    def forward(self, xray_image, y, a):
        # 使用骨齡模型提取 X-ray 特徵
        with torch.no_grad():
            out = self.bone_age_model(xray_image,y)
...
for gt_0_class in gt_0_classes:
    ...
    best_valLoss = 9999
    num_epochs = 200
    txtpath = f'./ablation/tall/xray+gender/train_ablation_xray_gender_subdataset{n}.txt'
    txtpath2 = f'./ablation/tall/xray+gender/val_ablation_xray_gender_subdataset{n}.txt'
    ...
    def saveModel():
        path = f"./ablation/tall/xray+gender/model_ablation_xray_gender_subdataset{n}.pth"
    ...
    for epoch in range(num_epochs):
        ...
        ff.flush()
    np.save(f'./ablation/tall/xray+gender/tall_model_{n}_train_metrics_ablation_xray_gender.npy', train_metrics_history)
    np.save(f'./ablation/tall/xray+gender/tall_model_{n}_val_metrics_ablation_xray_gender.npy', val_metrics_history)
